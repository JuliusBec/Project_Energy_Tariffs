...
print(f"[OK] Grundpreis: {enbw_data['base_price_monthly']:.2f} €/Monat")
print(f"[OK] Arbeitspreis (Zusatz-Komponenten): {enbw_data['markup_ct_kwh']:.2f} ct/kWh")
print(f" (Netzentgelte + Steuern + Umlagen + Herkunftsnachweise)")
print(f"[OK] Ø Börsenpreis (wird ersetzt): {enbw_data['exchange_price_ct_kwh']:.2f} ct/kWh")
print()

# Schritt 2: Erstelle DynamicTariff
print(" Schritt 2: DynamicTariff erstellen...")
print("-" * 80)
...
print(f"[OK] Tarif: {enbw_tariff.name}")
print(f"[OK] Grundpreis: {enbw_tariff.base_price:.2f} €/Monat")
print(f"[OK] Zusätzliche Komponenten: {enbw_tariff.additional_price_ct_kwh:.2f} ct/kWh")
print(f"[OK] Einmalige Netzgebühr: {enbw_tariff.network_fee:.2f} €")
print()
...
print(f"[OK] Jahresverbrauch: {ANNUAL_CONSUMPTION} kWh")
print(f"[OK] Gesamtkosten (30 Tage): {result['total_cost']:.2f} €")
print(f"[OK] Durchschnittlicher kWh-Preis: {result['avg_kwh_price']*100:.2f} ct/kWh")
print()
...
print(f" • Netzentgelte + Steuern (EnBW): {enbw_tariff.additional_price_ct_kwh:.2f} ct/kWh [OK]")
...
if abs(avg_price_ct - enbw_total) < 3:
 print(" [OK] SEHR GUT! Differenz < 3 ct/kWh")
elif abs(avg_price_ct - enbw_total) < 5:
 print(" [OK] GUT! Differenz < 5 ct/kWh")
else:
 print(" [WARNING] Größere Abweichung (normal, da EnBW Ø Börsenpreis 10.04 ct/kWh nutzt)")

print()
print("=" * 80)
print("[OK] INTEGRATION ERFOLGREICH")
print("=" * 80)
print()
print("Wichtig:")
