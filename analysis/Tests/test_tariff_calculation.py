...
from backend.energy_tariff import FixedTariff, DynamicTariff
...
def test_fixed_tariff():
 ...
 print(f"\n[OK] Result is within expected range (monthly cost)")
 elif abs(actual_cost - expected_annual_total) / expected_annual_total < 0.20:
 print(f"\n[WARNING] Result appears to be ANNUAL cost, not monthly!")
 else:
 print(f"\n[ERROR] Result is outside expected range!")
 ...
 print(f" [WARNING] BUG FOUND: Using annual consumption for monthly calculation!")
 print(f" Wrong calculation: ({annual_consumption} * {tariff.kwh_rate}) + {tariff.base_price} = €{wrong_calc:.2f}")

 except Exception as e:
 print(f"\n[ERROR] Error during calculation: {e}")
 import traceback
 traceback.print_exc()
