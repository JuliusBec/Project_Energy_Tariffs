...
def run_backtest_analysis(usage_df, show_daily=True, show_hourly=True):
 ...
 print(f"[WARNING] Removing incomplete last day ({last_day}) with only {hours_in_last_day} hours")
 backtest_df = backtest_df[backtest_df['datetime'].dt.date != last_day]
 complete_days = len(backtest_df) // 24
 print(f"[OK] Using {complete_days} complete days ({len(backtest_df)} hours) for backtest")
 ...
 print(f"[WARNING] Warning: Forecast has {len(forecast_only)} hours, backtest has {len(backtest_df)} hours")
 if len(forecast_only) < len(backtest_df):
 ...
 print(f"[OK] Extended forecast to {len(forecast_only)} hours to match backtest period")
...
def main():
 ...
 print("[ERROR] Error: CSV must have 'datetime' and 'value' columns")
