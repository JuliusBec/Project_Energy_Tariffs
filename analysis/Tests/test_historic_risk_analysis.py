...
def test_with_user_data():
 ...
 print(f"[ERROR] User data file not found: {user_data_path}")
 return

 print(f"\n[OK] Loading user data from: {user_data_path}")
 ...
 print(" [OK] The user has demonstrated good consumption behavior by using more")
 ...
 print("\n[OK] 7-day analysis completed successfully!")
 ...
 print(f"\n[ERROR] Error during 7-day analysis: {str(e)}")
...
def test_with_multiple_users():
 ...
 status = "[OK]" if r['exposure'] == 'favorable' else "⚠"
...
def test_coincidence_factor():
 ...
 print("\n[OK] Coincidence analysis completed successfully!")
 print(f"\nAnalysis Parameters:")
 print(f" Analysis period: {result['analysis_period']['start']} to {result['analysis_period']['end']}")
 print(f" Days analyzed: {result['days_analyzed']}")
 ...
 print(" [OK] Excellent! User demonstrates good consumption behavior by avoiding")
 ...
 print(f"\n[ERROR] Error during coincidence analysis: {str(e)}")
 import traceback
 traceback.print_exc()
 return
...
def test_coincidence_comparison():
 ...
 for r in results:
 rating_icon = "[OK]" if r['rating'] == 'low' else "⚠" if r['rating'] == 'medium' else "[X]"
 print(f"{rating_icon} {r['user']:<28} | {r['consumption_coincidence']:>13.2f}% | {r['cost_coincidence']:>8.2f}% | {r['correlation']:>12.4f} | {r['rating']}")

 print("\n" + "-"*80)
 print("Legend: [OK] = Good (low coincidence), ⚠ = Neutral (medium), [X] = Poor (high coincidence)")
 print("-"*80)
