 ...
 **Rückgabe:**
 ...
 print(f"[OK] Erkannte Zeitstempel-Spalte: '{time_col}'")
 print(f"[OK] Erkannte Verbrauchs-Spalte: '{value_col}'")
 ...
 print(f"[WARNING] {original_len - len(df)} Zeilen mit ungültigen Werten entfernt")
 ...
 print(f" [OK] Grundpreis: {scraped_data['base_price_monthly']:.2f} €/Mon")
 print(f" [OK] Zusatz-Komponenten: {scraped_data['additional_price_ct_kwh']:.2f} ct/kWh")
 ...
 print(f" [OK] Grundpreis: {scraped_data['base_price_monthly']:.2f} €/Mon")
 print(f" [OK] Zusatz-Komponenten: {scraped_data['markup_ct_kwh']:.2f} ct/kWh")

 except Exception as e:
 print(f" [ERROR] Fehler beim Scrapen von {provider}: {e}")
 import traceback
 traceback.print_exc()
 continue
 ...
 print(f" [OK] Durchschnitt: {result['avg_kwh_price']*100:.2f} ct/kWh")
 print(f" [OK] Monatliche Kosten: {result['total_cost']:.2f} €")

 except Exception as e:
 print(f" [ERROR] Fehler bei Berechnung für {provider_name}: {e}")
 import traceback
 traceback.print_exc()
 continue
 ...
 print(f" [OK] Fixpreis: {conv_tariff.kwh_rate*100:.2f} ct/kWh")
 print(f" [OK] Monatliche Kosten: {monthly_cost:.2f} €")

 except Exception as e:
 print(f" [ERROR] Fehler bei Berechnung für {conv_tariff.name}: {e}")
 import traceback
 traceback.print_exc()
 continue
 ...
 print(f"\n{'='*80}")
 print(f"[OK] VERGLEICH ABGESCHLOSSEN")
 print(f"{'='*80}\n")
...
async def get_price_forecast():
 ...
 print("[WARNING] No future data available, using most recent 168 hours")
 ...
 print(f"[OK] Loaded {len(forecast_data)} days of forecast data")
 ...
 import traceback
 print(f"[ERROR] Error loading forecast data: {str(e)}")
...
async def scrape_enbw_tariff(request: EnbwScraperRequest):
 ...
 logger.info(f"[OK] EnBW Scraping erfolgreich: {response.base_price_monthly} €/Mon, {response.markup_ct_kwh} ct/kWh (Quelle: {result.get('data_source')})")
...
async def scrape_tado_tariff(request: TadoScraperRequest):
 ...
 logger.info(f"[OK] Tado scraping erfolgreich: {response.base_price_monthly} €/Mon, {response.kwh_price_ct} ct/kWh (Quelle: {result.get('data_source')})")
 ...
 **Beispiel:**
 ...
 logger.info(f"[OK] Tibber-Scraper erfolgreich: {response.calculated_monthly_cost:.2f} €/Monat")
 return response

 except Exception as e:
 logger.error(f"[ERROR] Fehler beim Tibber-Scraping: {e}")
 raise HTTPException(
 status_code=500,
...
async def scrape_all_tariffs(
 ...
 logger.info(f"[OK] CSV loaded: {len(consumption_df)} rows")
 except Exception as e:
 logger.error(f"[ERROR] Error parsing CSV: {e}")
 consumption_df = None
 ...
 logger.info(f"[OK] EnBW: {result.get('base_price_monthly')} €/Mon, {result.get('markup_ct_kwh')} ct/kWh")
 else:
 ...
 tariffs.append(tariff_data)
 logger.info(f"[OK] EnBW Strom: {len(results)} Tarife hinzugefügt")
 else:
 ...
 tariffs.append(tariff_data)
 logger.info(f"[OK] Tado: {result.get('base_price_monthly')} €/Mon, {result.get('markup_ct_kwh')} ct/kWh markup (gesamt: {result.get('kwh_price_ct')} ct/kWh)")
 else:
 ...
 tariffs.append(tariff_data)
 logger.info(f"[OK] Tibber: {result.get('base_price_monthly')} €/Mon, {result.get('additional_price_ct_kwh')} ct/kWh")
 else:
 errors.append({"provider": "Tibber", "error": "No data returned"})

 except Exception as e:
 logger.error(f"[ERROR] Error scraping {provider}: {e}")
 ...
 usage_forecast_quality = backtest_data.get('metrics', {})
 logger.info(f"[OK] Backtest metrics calculated: CI width={usage_forecast_quality.get('relative_confidence_interval_width', 'N/A')}%")
 except Exception as e:
 # If backtest fails (e.g., not enough data), continue without forecast quality
 logger.warning(f"[WARNING] Could not calculate backtest metrics: {str(e)}")
 ...
 try:
 forecast_price_volatility = get_price_forecast_volatility(app_data_dir=app_data_dir)
 logger.info(f"[OK] Price forecast volatility calculated: σ={forecast_price_volatility.get('forecast_std_dev', 'N/A')} €/kWh")
 except Exception as e:
 logger.warning(f"[WARNING] Could not calculate price forecast volatility: {str(e)}")
 ...
 logger.info(f" [OK] {tariff['name']}: {risk_assessment['risk_level']} (score: {risk_assessment['risk_score']})")

 except Exception as e:
 logger.error(f" [ERROR] Error applying risk data for {tariff.get('name', 'unknown')}: {e}")
