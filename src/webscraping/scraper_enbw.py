...
class EnbwScraper:
 ...
 logger.info("[OK] Detail modal opened")
 except Exception as e:
 logger.warning(f"[WARNING] Could not click detail button: {e}")
 ...
 logger.info("[OK] Found modal content")
 except:
 pass
 ...
 logger.info(f"[OK] Found Arbeitspreis: {markup} ct/kWh")

 if boerse_match:
 exchange_price = float(boerse_match.group(1).replace(',', '.'))
 logger.info(f"[OK] Found Börsenpreis: {exchange_price} ct/kWh")
 ...
 logger.warning("[WARNING] Playwright loaded page but couldn't extract prices")
 return None
